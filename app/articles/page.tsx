  ...
  return (
    <div className="section-shell page-shell articles-page">
      <header className="page-heading">
        <h1>文章</h1>
        <p>技术教程、问题排查、项目复盘与学习笔记。优先说明问题边界、失败路径、方案取舍与验证方式。</p>
      </header>
        ...
        <div className="article-list archive-list">
            ...
            <article className="article-row" key={article.id}>
              <div>
                <div className="article-kicker">
                  ...
                  {article.series ? <span>系列 · {article.series}</span> : null}
                </div>
                <h2><Link href={`/articles/${article.slug}`}>{article.title}</Link></h2>
                <p>{article.summary}</p>
                <div className="tag-row">
                  {article.tags.map((tag, tagIndex) => (
                    <Link href={`/tags/${article.tag_slugs[tagIndex]}`} key={tag}>#{tag}</Link>
                  ))}
                </div>
              </div>
              <aside>
          ...
          {!articles.length ? (
            <div className="empty-state">
              <h2>还没有已发布文章</h2>
              <p>你可以先查看项目案例、能力地图或时间线。</p>
              <div className="hero-actions">
                <Link href="/projects">浏览项目</Link>
                <Link href="/stack">查看能力地图</Link>
              </div>
            </div>
          ) : null}
          {totalPages > 1 ? (
